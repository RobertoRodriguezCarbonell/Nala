  ...
  return (
    ...
    >
      {service ? (
        <div style={{ position: "relative" }}>
          ...
          >
            ...
            <span style={{ fontSize: "var(--text-md)", color: "var(--text-primary)", fontWeight: 500 }}>{service.name}</span>
            <span className="mono" style={{ fontSize: "var(--text-xs)", color: "var(--text-faint)" }}>·</span>
            <span className="mono" style={{ fontSize: "var(--text-sm)", color: "var(--method-post)" }}>{env?.name ?? "—"}</span>
            <ChevronIcon />
          </div>
            ...
            <>
              ...
              >
                {envs.map((e) => (
                  ...
                  >
                    <span className="mono" style={{ fontSize: "var(--text-sm)", color: "var(--text-secondary)" }}>{e.name}</span>
                    <span className="mono" style={{ fontSize: "var(--text-micro)", color: "var(--text-disabled)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 120 }}>
                      {e.baseUrl}
                    </span>
                  </div>
                ))}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--bg-input)", border: "0.5px dashed var(--border-control)", borderRadius: "var(--radius-control)", padding: "6px 11px", opacity: 0.7 }}>
          ...
          <span style={{ fontSize: "var(--text-md)", color: "var(--text-faint)" }}>Sin servicio</span>
        </div>
      )}
      ...
      <div style={{ flex: 1, maxWidth: 520, display: "flex", alignItems: "center", gap: 9, background: "var(--bg-input)", border: "0.5px solid var(--border-input)", borderRadius: "var(--radius-control)", padding: "6px 11px", opacity: 0.6 }}>
        <SearchIcon />
        <span style={{ fontSize: "var(--text-sm)", color: "var(--text-faint)", flex: 1 }}>Buscar endpoints, rutas, modelos…</span>
        <span className="mono" style={{ fontSize: "var(--text-micro)", color: "var(--text-faint)", border: "0.5px solid var(--border-control)", borderRadius: 4, padding: "1px 5px" }}>Ctrl K</span>
      </div>
      ...
      <div
        ...
        className="mono"
        style={{ height: 28, padding: "0 9px", borderRadius: "var(--radius-control)", border: "0.5px solid var(--border-input)", display: "flex", alignItems: "center", fontSize: "var(--text-xs)", color: "var(--text-muted)", cursor: "pointer" }}
